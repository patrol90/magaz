...
?>
					<tr class="type-shop_order status-<?php echo $order->post_status; ?>">
						<td class="order_status column-order_status" data-colname="Status">
							<mark class="<?php echo sanitize_title( $order->get_status() ); ?> tips" data-tip="<?php echo wc_get_order_status_name( $order->post_status ); ?>"><?php echo wc_get_order_status_name( $order->post_status ); ?></mark>
						</td>
						<td><a href="<?php echo admin_url( 'post.php?post=' . absint( $order->id ) . '&action=edit' ); ?>" class="row-title"><strong>#<?php echo esc_attr( $order->get_order_number() ); ?></strong></a></td>
						<td>
<?php
if ( '0000-00-00 00:00:00' == $order->order_date ) {
